...
public class RedshiftSQLSkin extends PostgresSQLSkin {
	...
	public RedshiftSQLSkin(ISkinProvider provider, DatabaseProduct product) {
	...
	public String render(SQLSkin skin, OperatorPiece piece,
		...
		if (opDef.getId()==IntrinsicOperators.CONCAT) {
			String result = super.render(skin, piece, opDef, args);
			// add a cast to avoid out-of-memory
			ExtendedType type = piece.getType();
			if (type.getSize()>0) {// better check ?
				return "CAST ("+result+" AS VARCHAR("+type.getSize()+"))";
			}
		}
		*/
		// else
