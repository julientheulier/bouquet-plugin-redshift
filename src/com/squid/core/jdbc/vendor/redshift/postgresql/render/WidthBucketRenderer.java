...
/**
 * Provide WIDTH_BUCKET render
 * @author nhadt
 *
 */
public class WidthBucketRenderer
extends BaseOperatorRenderer
{
	...
	public String prettyPrint(SQLSkin skin, OperatorDefinition opDef,
		...
		if (args != null) {
			if (args.length != 4)
				throw new RenderingException(
						"invalid syntax for EquiWidthBucket operator");
			metric = args[0];
			lowerBound = args[1];
			upperBound = args[2];
			bucketCount = args[3];
		...
		String str = "WIDTH_BUCKET(" + metric + "," + lowerBound + ","
				+ upperBound + "," + bucketCount + ")";
		return str;
	}
