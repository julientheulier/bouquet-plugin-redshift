...
public class SubStringRenderer
extends BaseOperatorRenderer
{
	...
	public String prettyPrint(SQLSkin skin, OperatorDefinition opDef, String[] args) throws RenderingException {
		...
		if (args != null) {
			if (!(args.length==2 || args.length==3)) throw new RenderingException("invalid syntax for SUBSTRING operator");
			principalstring = args[0];
		}
		String str = "SUBSTRING("+principalstring+" FROM "+args[1];
		if (args.length==3){
			str = str + " FOR "+args[2];
		}
		str = str + " )";
